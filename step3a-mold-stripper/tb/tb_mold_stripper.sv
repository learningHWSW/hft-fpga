// Self-checking TB for mold_stripper, chained into step 2's itch_decoder.
//
// Reads a .mold file (+mold=<path>, default test.mold; 2B BE length-prefixed
// UDP payloads from gen_itch.py --mold), drives each payload as one
// AXI-Stream packet honoring s_tready, and logs to mold_rtl.log:
//   - decoded messages (same canonical format as step 2 — log_msg copied)
//   - GAP / HB / EOS event lines from the stripper
// The Makefile diffs that log against scripts/dump_mold.py output (golden).
// Event pulses and decoder valids never coincide (header parse and decode
...
module tb_mold_stripper;
  ...
  mold_stripper #(.DATA_W(DATA_W)) dut (
    .clk          (clk),
    .rst_n        (rst_n),
    .s_tdata      (s_tdata),
    .s_tkeep      (s_tkeep),
  ...
  // ---------- monitor: canonical log (messages + events) ----------
  int fd_log;
  int n_decoded = 0;
  int n_len_err = 0;
  bit eos_seen = 1'b0;

  initial fd_log = $fopen("mold_rtl.log", "w");
  ...
  always @(posedge clk) begin
    ...
    if (ev_gap)
      $fdisplay(fd_log, "GAP expected=%0d got=%0d missing=%0d",
                ev_expected, ev_seq, ev_seq - ev_expected);
    if (ev_hb)  $fdisplay(fd_log, "HB seq=%0d", ev_seq);
    if (ev_eos) begin
      $fdisplay(fd_log, "EOS seq=%0d", ev_seq);
      eos_seen <= 1'b1;
    end
  end
  ...
  task automatic send_frame(input int n);
    ...
    while (i < n) begin
      ...
      s_tvalid = 1'b1;
      s_tlast  = (i + k == n);
      // beat is accepted on the first posedge with s_tready high
      forever begin
        @(posedge clk);
        if (s_tready) break;
      end
  ...
  initial begin
    ...
    forever begin
      ...
      send_frame(len);
      // mix back-to-back packets with idle gaps
    ...
    $display("stripper: gap_total=%0d dup=%0d frame_err=%0d eos=%0d",
             gap_total, dup_cnt, frame_err_cnt, eos_seen);
    if (n_len_err != 0 || frame_err_cnt != 0 || !eos_seen) begin
