// End-to-end TB for the WIRE path: Ethernet frames -> eth_ip_udp_rx -> fh_core -> BBO.
//
...
//
// +eth=<path>, +loc=<n>, +base=<n>, +gap=<n>.
`timescale 1ns/1ps

module tb_wire_to_bbo;
  import itch5_pkg::*;
  ...
  // wire -> parser -> feed handler
  logic [DATA_W-1:0] p_tdata;
  logic [KEEP_W-1:0] p_tkeep;
  logic              p_tvalid, p_tlast;
  logic [31:0] frames_in, frames_kept, drop_not_ipv4, drop_not_udp, drop_group;

  eth_ip_udp_rx #(.DATA_W(DATA_W)) rx (
    .clk(clk), .rst_n(rst_n),
    .cfg_group_ip(32'hE9360C01),      // 233.54.12.1
    ...
    .frames_in(frames_in), .frames_kept(frames_kept),
  ...
  initial begin
    ...
    $display("  parser   : in=%0d kept=%0d drop(ipv4=%0d udp=%0d group=%0d)",
             frames_in, frames_kept, drop_not_ipv4, drop_not_udp, drop_group);
