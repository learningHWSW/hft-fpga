...
| target | removes | cost to recreate |
|---|---|---|
| `make clean` | logs, simulator scratch (`.wdb`, `.jou`, `.pb`, `xsim.dir`), small captures, metadata orphaned by a removed bitstream | seconds to minutes |
| `make clean-build` | packaged kernels, `.xo`, the `_x_*` v++ temp directories, stale reports | minutes |
| `make distclean` | `*.xclbin`, `ip/`, `real_replay.bin` — **prompts first** | **hours** |

This used to be one target, and the split is not tidiness. The single `clean`
removed `*.xclbin` in the same breath as `*.log`, so tidying up logs silently cost
two 1 h 10 m links — including the Phase B bitstream behind the only wire-to-wire
figure in this repository. It also deleted `host/t2t_regs.h`, which is *generated*
but also *tracked*, so a clean left the working tree dirty and looked like a
source file had gone missing.

**No tier removes a tracked file.** `host/t2t_regs.h` is regenerated by its own
rule when it goes out of date, so there is no reason for a clean to touch it.

Two things `clean` deliberately keeps, because both are cheap to store and
expensive to rebuild: `ip/` (a Vivado IP generation, `make cmac`) and
`real_replay.bin` (312 MB, packed from the full-day ITCH capture).
